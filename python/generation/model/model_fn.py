...
def build_model(mode, inputs, params, sentence_max_len=None):
    ...
    if params.model_version == 'lstm':
        ...
        cell = tf.nn.rnn_cell.MultiRNNCell([new_cell(params.lstm_num_units) for _ in range(params.lstm_num_layers)])
...
def model_fn(mode, inputs, params, reuse=False):
    ...
    with tf.variable_scope("metrics"):
        metrics = {
            'accuracy': tf.metrics.accuracy(labels=tf.boolean_mask(labels, mask),
